...
background = raster("Background_rast.asc"); envVariables = list()
raster_files = read.table("Environmental_rasters/Brazil_environmental_rasters_list.txt", head=F)
for (i in 1:dim(raster_files)[1]) envVariables[[i]] = raster(paste0("Environmental_rasters/",raster_files[i,1],".asc"))
envVariables_mod = list(); templateRaster = envVariables[[20]] # resolution: 0.0833 x 0.0833
...
envVariableNames = gsub("Brazil_","",gsub("\\.asc","",gsub("_mask","",gsub("_crop_","_",raster_files[,1]))))
...
raster_names[7] = "Culex quinquefasciatus ecological suitability"; colour_scales[[7]] = rev(colorRampPalette(brewer.pal(11,"RdYlBu"))(101))
raster_names[8] = "C. paraensis suitability"; colour_scales[[8]] = rev(colorRampPalette(brewer.pal(11,"RdYlBu"))(101))
raster_names[9] = "A. aegypti ecological suitability"; colour_scales[[9]] = rev(colorRampPalette(brewer.pal(11,"RdYlBu"))(101))
raster_names[10] = "Soil moisture"; colour_scales[[10]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[11] = "Decidious broadleaf forests"; colour_scales[[11]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[12] = "Evergreen broadleaf forests"; colour_scales[[12]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[13] = "Mixed trees forests"; colour_scales[[13]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[14] = "Grasslands"; colour_scales[[14]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[15] = "Shrublands"; colour_scales[[15]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[16] = "Savannas"; colour_scales[[16]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[17] = "Woody savannas"; colour_scales[[17]] = colorRampPalette(brewer.pal(9,"YlGn"))(101)
raster_names[18] = "Croplands & natural vegetation"; colour_scales[[18]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[19] = "Croplands"; colour_scales[[19]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[20] = "Banana harvested area (log)"; colour_scales[[20]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[21] = "Cassava harvested area (log)"; colour_scales[[21]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[22] = "Cocoa harvested area (log)"; colour_scales[[22]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[23] = "Coffee harvested area (log)"; colour_scales[[23]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[24] = "Maize harvested area (log)"; colour_scales[[24]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[25] = "Soybean harvested area (log)"; colour_scales[[25]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[26] = "Sugarcane harvested area (log)"; colour_scales[[26]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[27] = "Cattle production"; colour_scales[[27]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[28] = "Forest loss 2020-2023"; colour_scales[[28]] = colorRampPalette(brewer.pal(9,"YlOrBr"))(111)[1:101]
raster_names[29] = "Waterbodies"; colour_scales[[29]] = colorRampPalette(brewer.pal(9,"Blues"))(101)
raster_names[30] = "Mean annual precipitation"; colour_scales[[30]] = colorRampPalette(brewer.pal(9,"Blues"))(101)
raster_names[31] = "Mean annual temperature"; colour_scales[[31]] = colorRampPalette(brewer.pal(9,"Reds"))(131)[1:101]
...
for (i in 1:length(beast_analyses))
	{
		files = list.files(paste0("Tree_extraction_files/",localTreesDirectories[i],"_ext/"))
		files = files[which(grepl("TreeExtractions_",files))]; tMRCAs = rep(NA, length(files))
		for (j in 1:length(files))
			{
				tab = read.csv(paste0("Tree_extraction_files/",localTreesDirectories[i],"_ext/",files[j]), head=T)
				tMRCAs[j] = tab[which(!tab[,"node1"]%in%tab[,"node2"])[1],"startYear"]
			}
		HPD = round(HDInterval::hdi(tMRCAs)[1:2],2); medianV = round(median(tMRCAs),2)
		cat("Segment ",segments[i],": tMRCA = ",medianV,", 95% HPD = [",HPD[1],", ",HPD[2],"]\n",sep="")
	}
...
for (i in 1:length(beast_analyses))
	{
		...
		outputName = paste0("All_dispersal_statistics/Sliding_window_1y/OROV_segment",segments[i])
...
for (i in 1:length(beast_analyses))
	{
		mat = read.table(paste0("All_dispersal_statistics/Sliding_window_1y/OROV_segment",segments[i],"_estimated_dispersal_statistics.txt"), head=T)
...
selected_variables = which(raster_names%in%c("Evergreen broadleaf forests","Population density","A. aegypti ecological suitability",
											 "Banana harvested area (log)","Cocoa harvested area (log)","Mean annual temperature"))
...
pdf(paste0("Dispersal_history_4_NEW.pdf"), width=9, height=5); directory_dispersal_stats = "Sliding_window_2m"
par(mfrow=c(3,3), oma=c(0,0,0,0), mar=c(2.5,2.5,0,0), mgp=c(0,0.1,0), lwd=0.3, bty="o", col="gray30")
for (i in 1:length(beast_analyses))
	{
		swf_median = read.table(paste0("All_dispersal_statistics/",directory_dispersal_stats,"/OROV_segment",segments[i],"_median_spatial_wavefront_distance.txt"), header=T)
		swf_95pHPD = read.table(paste0("All_dispersal_statistics/",directory_dispersal_stats,"/OROV_segment",segments[i],"_95%HPD_spatial_wavefront_distance.txt"), header=T)
...
for (i in 1:length(beast_analyses))
	{
		wdc_median = read.table(paste0("All_dispersal_statistics/",directory_dispersal_stats,"/OROV_segment",segments[i],"_median_weighted_diffusion_coefficient.txt"), header=T)[,1:2]
		wdc_95pHPD = read.table(paste0("All_dispersal_statistics/",directory_dispersal_stats,"/OROV_segment",segments[i],"_95%HPD_weighted_diffusion_coefficient.txt"), header=T)
		...
		yMin = min(wdc[,"95pHDP_lower"], na.rm=T); yMax = max(wdc[,"95pHDP_upper"], na.rm=T); yMin = 0; yMax = 6500
		...
		for (j in 1:length(wdc[,"time"]))
			{
				...
				y2 = wdc[j,"95pHDP_upper"]+(2000000/365.25); y1 = 0 # wdc[j,"95pHDP_lower"]-(500000/365.25); 
				polygon(c(x1,x2,x2,x1), c(y1,y1,y2,y2), col="gray90", border=NA)
				polygon(c(x1,x2,x2,x1), c(y1,y1,y2,y2), col=colours[j], border=NA)
			}
		getOption("scipen"); opt = options("scipen"=20); polygon(xx_l,yy_l,col=NA,border="gray30")
		...
		for (j in c(2021:2024)) abline(v=j, lty=2, col="gray30", lwd=0.3)
		axis(side=1, lwd.tick=0.3, cex.axis=1.0, lwd=0.3, tck=-0.030, col="gray30", col.axis="gray30", col.tick="gray30", mgp=c(0,0.35,0), at=seq(2020,2025))
		axis(side=2, lwd.tick=0.3, cex.axis=1.0, lwd=0.3, tck=-0.030, col="gray30", col.axis="gray30", col.tick="gray30", mgp=c(1,0.35,0), at=c(0,2000,4000,6000,8000))
...
for (i in 1:length(beast_analyses))
	{
		...
		# dev.new(); hist(log(((geo_dists^2)/(4*durations))), breaks=1000, col="gray50", border=NA); abline(v=7.5, col="red")
		# dev.new(); hist(log(geo_dists), breaks=1000, col="gray50", border=NA); abline(v=1.65, col="red")
		# dev.new(); plot(log(geo_dists), log(((geo_dists^2)/(4*durations))), col="gray50", cex=0.5)
		# abline(v=3, col="red"); abline(v=8.3, col="red"); abline(h=7.5, col="red"); abline(h=20, col="red")		
		...
		xLab = "geographic distance (km, log)"; yLab = "branch diffusion coeffcient (km2/year, log)"
		plot(kde, display="filled.contour2", cont=c(50,75,95), col=three_colours, axes=F, ann=F, xlim=c(-4.5,8.3), ylim=c(-6.2,17.5))
		# plot(kde, display="filled.contour2", cont=c(50,75,95), col=three_colours, axes=T, ann=F, xlim=c(-4.5,8.3), ylim=c(-6.2,17.5))
		# abline(v=3, col="red"); abline(h=7.3, col="red") # corresponding to ~20 km and ~1500 km2/year or 4 km2/day		
